...
async function callGemini({ providerConfig, messages, systemPrompt, hasImage, promptType, useWebSearch, signal }) {
  ...
  if (!response.ok) {
    ...
    console.error('[Gemini] HTTP', response.status, errMsg, JSON.stringify(data)?.slice(0, 400));
  ...
  if (!text) {
    ...
    console.error('[Gemini] empty response blockReason:', blockReason, 'finishReason:', data?.candidates?.[0]?.finishReason);
