...
function parseCommunityVotes(ratingValueRaw, ratingCountRaw) {
  ...
  if (!baseCount) {
    return { strong: 0, balanced: 0, light: 0 };
  }
...
function buildPerfumeRow(row) {
  ...
  const communityVotes = parseCommunityVotes(row.Rating_Value || row.rating, row.Rating_Count || row.rating_count);
...
export async function importParfumo(csvPath) {
  ...
  const perfumes = rows.map(buildPerfumeRow).filter(Boolean);
  ...
  for (let i = 0; i < perfumes.length; i += BATCH_SIZE) {
    ...
    if (error) {
      throw new Error(`Batch ${Math.floor(i / BATCH_SIZE) + 1} import hatasi: ${error.message}`);
    }
...
if (isDirectRun) {
  ...
  if (!fileArg) {
    console.error('Kullanim: node scripts/import-parfumo.mjs <csv-path>');
    process.exit(1);
  }
