...
function serializeEvidenceRelations(relations) {
  return (Array.isArray(relations) ? relations : [])
    .map((relation) => {
      if (!relation?.fragrance_id || !relation?.molecule_id) return null;
      return {
        id: String(relation.id || `${relation.fragrance_id}:${relation.molecule_id}`),
        ...
        evidence_level: relation.evidence_level || 'unmatched',
        evidence_label: relation.evidence_label || '',
        evidence_reason: relation.evidence_reason || '',
        matched_notes: Array.isArray(relation.matched_notes) ? relation.matched_notes : [],
        note_roles: Array.isArray(relation.note_roles) ? relation.note_roles : [],
        is_iconic: Boolean(relation.is_iconic),
        percentage: Number.isFinite(Number(relation.percentage)) ? Math.round(Number(relation.percentage)) : 0,
      };
    })
    .filter(Boolean);
}
...
async function seedCatalog(options) {
  ...
  if (Array.isArray(relations) && relations.length > 0) {
    const evidenceRows = serializeEvidenceRelations(relations);
    const relationResult = await supabase.from(evidenceTable).upsert(evidenceRows, {
      onConflict: 'fragrance_id,molecule_id',
    });

    if (!relationResult.error) {
      evidenceRelationCount = evidenceRows.length;
    } else if (!/could not find the table|relation .* does not exist/i.test(relationResult.error.message || '')) {
