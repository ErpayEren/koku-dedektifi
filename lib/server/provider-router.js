...
async function callGemini({ providerConfig, messages, systemPrompt, hasImage, promptType, useWebSearch, signal }) {
  ...
  for (const model of modelChain) {
    ...
    if (!response.ok) {
      ...
      console.error('[Gemini] HTTP', response.status, errMsg, JSON.stringify(data)?.slice(0, 400), 'model=', model);
      lastError = { ok: false, status: response.status, error: errMsg };

      // Model-specific quota or temporary load: try next Gemini model before failing.
      if ([429, 503, 529].includes(Number(response.status))) {
        continue;
      }

      // Schema/model compatibility issue: retry once without strict schema contract on this model.
  ...
  // All models exhausted with primary key — try backup key on 429
  if (lastError?.status === 429 && providerConfig.backupApiKey) {
    console.log('[Gemini] tüm modeller primary key 429 verdi — backup key deneniyor');
    const backupResponse = await fetch(
      `https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(primaryModel)}:generateContent`,
      {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'x-goog-api-key': providerConfig.backupApiKey,
        },
        body: JSON.stringify(payload),
        signal,
      },
    );
    const backupData = await readJsonSafely(backupResponse);
    if (backupResponse.ok) {
      const backupText =
        backupData?.candidates?.[0]?.content?.parts?.map((part) => part?.text || '').join('').trim() || '';
      if (backupText) {
        return {
          ok: true,
          formatted: {
            content: [{ type: 'text', text: backupText }],
            usage: { output_tokens: backupData?.usageMetadata?.candidatesTokenCount || 0 },
            model: primaryModel,
            annotations: backupData?.candidates?.[0]?.groundingMetadata || null,
          },
        };
      }
    }
    console.error('[Gemini] backup key de başarısız:', backupResponse.status);
  }
