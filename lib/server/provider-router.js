    ...
    gemini:
      geminiEnvKey || genericGeminiKey
        ? {
            ...
            backupApiKey: cleanString(process.env.GEMINI_API_KEY_2) || null,
          }
        : null,
...
function getProviderConfig() {
  ...
  const fallbackOrder = ['gemini', 'openrouter', 'anthropic'];
  const firstAvailable = fallbackOrder.map((name) => configs[name]).find(Boolean);

  if (preferred) {
    if (configs[preferred]) return configs[preferred];
    if (firstAvailable) {
      console.error(
        '[Error] preferred provider unavailable, falling back:',
        preferred,
        '->',
...
// Converts JSON Schema (draft-07 array union types) to Gemini-compatible OpenAPI format.
// Gemini rejects type: ["string","null"] — needs type: "string", nullable: true.
function toGeminiSchema(schema) {
  if (!schema || typeof schema !== 'object' || Array.isArray(schema)) return schema;
  const out = Object.assign({}, schema);
  if (Array.isArray(out.type)) {
    const nonNull = out.type.filter((t) => t !== 'null');
    const hadNull = out.type.length !== nonNull.length;
...
function getGeminiPayload({ messages, systemPrompt, hasImage, promptType, useWebSearch, providerConfig, responseJsonSchema }) {
  ...
  const payload = {
    ...
    generationConfig: {
      temperature: promptType === 'analysis' ? 0.0 : 0.7,
      responseMimeType: promptType === 'analysis' ? 'application/json' : 'text/plain',
      maxOutputTokens: promptType === 'analysis' ? 2000 : 1600,
      thinkingConfig: { thinkingBudget: 0 },
    },
  };

  if (promptType === 'analysis') {
    payload.generationConfig.responseSchema = toGeminiSchema(responseJsonSchema || ANALYSIS_JSON_SCHEMA);
  }
...
function getOpenRouterPayload({ messages, systemPrompt, hasImage, promptType, useWebSearch, providerConfig }) {
  ...
  const payload = {
    model,
    max_tokens: promptType === 'analysis' ? 1200 : (useWebSearch ? 2400 : 1600),
...
function getAnthropicPayload({ messages, systemPrompt, promptType, useWebSearch, providerConfig }) {
  const payload = {
    ...
    max_tokens: promptType === 'analysis' ? 1200 : (useWebSearch ? 3000 : 1600),
    system: systemPrompt,
...
function isRetryableResult(result) {
  ...
  // 429 excluded: callGemini already exhausts all keys+models internally; outer retry just wastes quota
  if ([408, 409, 425, 500, 502, 503, 504, 529].includes(status)) return true;
...
// Round-robin counter: her çağrıda farklı key ile başlar, tek key'i art arda doldurmaz
let _geminiKeyRoundRobin = 0;
...
async function callGemini({ providerConfig, messages, systemPrompt, hasImage, promptType, useWebSearch, signal, responseJsonSchema }) {
  const timeoutSignal = AbortSignal.timeout(GEMINI_TIMEOUT_MS);
  const effectiveSignal = signal
    ? AbortSignal.any([signal, timeoutSignal])
    : timeoutSignal;

  const { model: primaryModel, payload } = getGeminiPayload({
    messages,
    systemPrompt,
    hasImage,
    promptType,
  ...
  if (hasImage) {
    const allParts = (payload.contents || []).flatMap((c) => c.parts || []);
    const imagePart = allParts.find((p) => p.inline_data);
    console.log('[IMAGE] Gemini request parts count:', allParts.length);
    if (imagePart) {
      console.log('[IMAGE] mimeType:', imagePart.inline_data.mime_type);
      console.log('[IMAGE] imageBase64 length:', imagePart.inline_data.data?.length);
    } else {
      console.log('[IMAGE] imageBase64 length: 0 (inline_data bulunamadi — imageBase64 bos veya parse hatasi)');
      console.log('[IMAGE] mimeType: undefined');
    }
  }

  const configuredFallbackModels = cleanString(process.env.GEMINI_FALLBACK_MODELS)
    .split(',')
  ...
  // Round-robin key selection: alternate between primary and backup key across calls
  const keys = [providerConfig.apiKey, providerConfig.backupApiKey].filter(Boolean);
  const startKeyIdx = keys.length > 1 ? (_geminiKeyRoundRobin++ % keys.length) : 0;
  console.log('[KeyRotation] key1 defined:', !!providerConfig.apiKey, 'key2 defined:', !!providerConfig.backupApiKey, 'totalKeys:', keys.length, 'startKeyIdx:', startKeyIdx);

  let lastError = null;

  for (let keyAttempt = 0; keyAttempt < keys.length; keyAttempt++) {
    const keyIdx = (startKeyIdx + keyAttempt) % keys.length;
    const apiKey = keys[keyIdx];
    console.log('[KeyRotation] using key:', keyIdx + 1, 'of', keys.length);

    if (keyAttempt > 0) {
      console.log('[Gemini] 429 — key rotasyonu, 5000ms bekleniyor');
      await delay(5000);
    }

    let allModelsHit429 = true;
    ...
    for (const model of modelChain) {
      ...
      try {
        ...
        if (fetchErr?.name === 'AbortError' || fetchErr?.name === 'TimeoutError') {
          console.error('[Gemini] timeout after', GEMINI_TIMEOUT_MS, 'ms — model=', model);
          return { ok: false, status: 504, error: `Gemini ${GEMINI_TIMEOUT_MS}ms timeout` };
        }
        throw fetchErr;
      }
      ...
      if (!response.ok) {
        const errMsg = mapProviderError('gemini', response, data);
        console.error('[Gemini] HTTP', response.status, errMsg, 'model=', model, 'keySlot=', keyIdx, 'body=', JSON.stringify(data)?.slice(0, 300));
        lastError = { ok: false, status: response.status, error: errMsg };

        if ([429, 503, 529].includes(Number(response.status))) {
          continue; // try next model with same key
        }

        // Non-429 error: don't rotate key for this
        allModelsHit429 = false;

        // Schema/model compatibility issue: retry once without strict schema on this model
        if (Number(response.status) === 400 && promptType === 'analysis') {
          const relaxedPayload = JSON.parse(JSON.stringify(payload));
          if (relaxedPayload.generationConfig) {
            delete relaxedPayload.generationConfig.responseSchema;
            delete relaxedPayload.generationConfig.responseMimeType;
          }
          ...
          const relaxedData = await readJsonSafely(relaxedResponse);
          if (relaxedResponse.ok) {
            const relaxedText =
              relaxedData?.candidates?.[0]?.content?.parts?.map((part) => part?.text || '').join('').trim() || '';
            if (relaxedText) {
              return {
                ok: true,
                formatted: {
                  content: [{ type: 'text', text: relaxedText }],
                  usage: { output_tokens: relaxedData?.usageMetadata?.candidatesTokenCount || 0 },
                  model,
                  annotations: relaxedData?.candidates?.[0]?.groundingMetadata || null,
                },
              };
            }
          }
      ...
      if (!text) {
        ...
        const errMsg = blockReason ? `Gemini yaniti engelledi: ${blockReason}` : 'Gemini bos yanit dondu.';
        console.error('[Gemini] empty response blockReason:', blockReason, 'finishReason:', data?.candidates?.[0]?.finishReason, 'model=', model);
        lastError = { ok: false, status: 502, error: errMsg };
        continue;
      }
    ...
    // If not all models returned 429, no point trying next key
    if (!allModelsHit429) break;
  }

  return lastError || { ok: false, status: 502, error: 'Gemini yaniti alinamadi.' };
}
...
async function callAIProvider(messages, promptType, apiKeys) {
  ...
  let providerChain = [providerConfig, ...getProviderChain(providerConfig.provider)];
  const { configs } = resolveProviderConfigs();

  // Image analysis is most reliable on Gemini in this project.
  // Always put Gemini at the head of the chain for image requests.
  if (Boolean(apiKeys.hasImage) && configs.gemini) {
    providerChain = [configs.gemini, ...providerChain.filter((item) => item?.provider !== 'gemini')];
  }
  console.log(
    '[callAIProvider] chain:',
    providerChain.map((p) => p?.provider).join(' -> ') || 'EMPTY',
    '| hasImage:', Boolean(apiKeys.hasImage),
  );

  let firstFailure = null;   // first provider that failed — root cause for error reporting
  ...
  for (const candidate of providerChain) {
    ...
    if (!firstFailure) firstFailure = candidateFailure;
    lastFailure = candidateFailure;

    const normalizedError = cleanString(result?.error).toLowerCase();
    const status = Number(result?.status || 0);

    // Treat Gemini auth / quota errors as dominant so they surface over downstream failures.
    // 400 = expired key, 401 = invalid key, 402/429/503/529 = quota/rate-limit.
    const isGeminiAuthOrQuotaLike =
      candidate.provider === 'gemini' &&
      ([400, 401, 402, 429, 503, 529].includes(status) ||
        ...
        normalizedError.includes('gecersiz'));

    if (isGeminiAuthOrQuotaLike && !dominantFailure) {
      dominantFailure = candidateFailure;
    }
  }
