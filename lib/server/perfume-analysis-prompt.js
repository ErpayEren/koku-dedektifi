...
async function findPerfumeContextByInput(inputText, options = {}) {
  ...
  try {
    ...
    const selectColumns =
      'id, name, brand, year, concentration, gender_profile, seasons, occasions, longevity_score, sillage_score, top_notes, heart_notes, base_notes, accords, rating, price_tier';
    const { data: rows, error } = await client
      .from(table)
      .select(selectColumns)
      .or(orFilters.join(','))
