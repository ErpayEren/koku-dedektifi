...
async function pingSupabaseWardrobe(config) {
  if (!config.url || !config.serviceRoleKey) {
    return { ok: false, reason: 'supabase_missing' };
  }

  const url = `${config.url}/rest/v1/${encodeURIComponent(config.wardrobeTable)}?select=user_id&limit=1`;
...
async function handler(req, res) {
  ...
  const requireSupabase = shouldRequireSupabaseWardrobe();
  const runtimeFallbackAllowed = allowRuntimeFallback();

  let ping = { ok: false, reason: 'skipped' };
  if (configured) {
    try {
      ping = await pingSupabaseWardrobe(supabase);
    } catch (error) {
      ping = {
        ok: false,
